...
\textit{\textbf{Lemma 5.} Let $(\pi_A, \chi_A)$ be a velvet suffix proof
constructed by adversary and $b_s$ generated at round $r_s$ the most recent
smooth block in the proof. Then $\forall r: r \leq r_s$ no thorny blocks
generated at round $r$ can be included in $(\pi_A, \chi_A)$.} \\ 
\textit{Proof.} By contradiction. Let $b_t$ be a thorny block generated at
some sound $r_t \leq r_s$. Consider $b_t$ is included in proof. Then, because
$(\pi_A, \chi_A)$ is a valid chain as for interlink pointers, there exist a
block $b'$ generated at some round $r'$ such that $r_t \leq r' \leq r_s$,
which has an interlink pointer to $b_t$. If $b'$ is a smooth block we have
reached contradiction, since than there can be no interlink pointer to $b_t$
as it contains only smooth pointers. Now take the case that $b'$ is a thorny
block. Then because $b'$ was generated in round $r' \leq r_s$, by induction
there should be a thorny block pointed by $b_s$ which is not possible because
$b_s$ is a smooth block.
...
\begin{algorithm}[H]
	...
	\vspace{4mm}
	\Fn{is-smooth-block(B)}{
		\If{B.id $=$ Genesis.id}{
			\Return true\;
		}
 		\For{p $\in$ B.interlink} {
     		\If{is-smooth-pointer(B, p) $=$ false}{
     			\Return false\;
     		}
 		}
 		\Return true\;
...
In conclusion Verify algorithm for the NIPoPoW suffix protocol remains the same.
