...
This claim is described and discussed in the following sections.
Note that any reference to adversarial party bound is respect to the number of
upgraded parties. Therefore, ``(1/2)-bounded adversary" holds for
$\dfrac{t}{n_h} < (1 - \delta) $, while ``(1/3)-bounded adversary" holds for
$\dfrac{t}{n_h} < \dfrac{1 - \delta}{2} $.
...
We describe a protocol patch that operates as follows. The NIPoPoW protocol under velvet fork works as usual but each miner constructs smooth blocks. This means that  a block's interlink is constructed excluding thorny blocks (except the
