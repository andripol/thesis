...
\textbf{Claim 1:} $\alpha_A \downarrow \uparrow^{\mu_A}, \alpha'_B\downarrow$ are mostly disjoint. We show this by taking the two possible cases for the relation of $\mu_A$, $\mu'_B$ and the cases of $\alpha_A$ containing blocks with correct or incorrect interlinks up until block $b_2 = LCA(\alpha_A \downarrow \uparrow^{\mu_A}, \alpha'_B\downarrow)$.\\
This means that $b_2$ is the LCA block of adversary's proof and honest player's 0-level chain, without considering any (adversarial) blocks belonging in $\alpha'_B \downarrow$ sewed in the adversary's proof.


\textit{\underline{Claim 1a}:} If no adversarially generated block of chain $C_B$ with incorrect interlink is sewed in $\alpha_A$ then the claim's proof is the same to that of Theorem 2, thus $k_1 \leq 2^{\mu'_B - \mu_A}$.

\textit{\underline{Claim 1b}:} If a block $b' = \{b' \in C_B\{b:b_2\} \cap \textit{ } b' \textit{ contains incorrect interlink} \}$ is included in $\alpha_A$ then no more honestly generated block of the common subchain $C_B\{b':b_2\}$ is included in $\alpha_A$ because of Remark 2. 
In that case, blocks in $\alpha_A\{b:b'\}$ are subject to the constraints of Claim 1a, thus we have $k_{1a} \leq 2^{\mu'_B - \mu_A}$. 
Blocks in $\alpha_A\{b':b_2\}$ are adversarially generated either directly in $C_A$ either sewed from $C_B$. Thus from that point on it is equivalent considering two disjoint chains competing each other. Let these blocks count to $k_{1b}$. \textit{(Consider the rounds when these blocks were generated and apply Lemma 1 to get a desirable inequality)}\\

Finally we have 
$\vert \alpha_A\downarrow\uparrow^{\mu_A}[1:] \cap \alpha'_B\downarrow[1:] \vert = k_{1a} + k_{1b} = k_1$, where $k_{1b} \leq C_B\{b':b_2\}\uparrow^{\mu_A}$.\\

From Claim $1a$ and Claim $1b$, we conclude that there are $\vert \alpha_A \vert - k_1$ blocks after block $b$ in $\alpha_A$ which are not honestly generated blocks existing in $\alpha_B\downarrow$. In other words, there are $\vert \alpha_A \vert - k_1$ blocks after block $b$ in $\alpha_A$ which are either adversarially generated existing in $\alpha_B\downarrow$ either don't belong in $\alpha_B\downarrow$. This makes $b_2$ the last block before the 0-level fork point included in the adversary's proof.\\

\textbf{Claim 2.} To be constructed. (again, once an "incorrect" block is included in the proof no other honestly generated block can be included because of Remark 1. Just as Claim 1, in parallel to the proof of theorem 2.)\\
