...
Let round $r_1$ the round when block $b$ is generated and round $r_2$ when block $b_2 = LCA(\alpha_A \downarrow \uparrow^{\mu_A}, \alpha'_B\downarrow)$ is generated.
...
\textit{\underline{Claim 1a}:} If no adversarially generated block with incorrect interlink is included in $\alpha_A$ then the proof is the same to that of Theorem 2, thus $k_1 \leq 2^{\mu'_B - \mu_A}$.

\textit{\underline{Claim 1b}:} If a block $b' = \{b' \in C_B\{b:b_2\} \cap \textit{ } b' \textit{ contains incorrect interlink} \}$ is included in $\alpha_A$ then no more honestly generated block of the common subchain $C_B\{b:b_2\}$ is included in $\alpha_A$ because of Remark 2. \\ In that case, blocks in $\alpha_A\{b:b'\}$ are subject to the constraints of Claim 1a, thus we have $k_{1a} \leq 2^{\mu'_B - \mu_A}$. Blocks in $\alpha_A\{b':b_2\}$ are adversarially generated and contain incorrect interlinks thus from that point on it is equivalent considering two disjoint chains competing each other. Let these blocks count to $k_{1b}$. \textit{(Consider the rounds when these blocks were generated and apply Lemma 1 to get the inequality)}\\
