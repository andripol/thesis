...
honestly generated block included in the suffix proof. Obviously, there is a case
...
We need to bound the number of rounds a client has to wait until the block of interest
is buried under sufficient number of blocks so that he can obtain the infix proof
needed with high probability.

Let $p_H$ the probability that a block is generated by an honest party during a round.
Then considering that collisions to RO happen with negligible probability we have
...
Then we have that $N_{BA}$, $N_{BH}$ follow the Binomial Distribution.
