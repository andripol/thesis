...
By contradiction. Let $m = k_1 + k_2 + k_3$ and let $k_1, k_2, k_3$ be polynomial functions of $\kappa$. Let $Q$ be a $k-$stable monotonic suffix-sensitive chain predicate. Assume NIPoPoWs on $Q$ is insecure. Then, during an execution of at some round  $r_3$, $Q(C)$ is defined and the verifier $V$ disagrees with some honest participant. Assume the execution is typical. $V$ communicates with adversary $A$ and honest prover $B$. The verifier receives proofs $\pi_A, \pi_B$. Because $B$ is honest, $\pi_B$ is a proof constructed based on underlying blockchain $C_B$ (with $\pi_B \subseteq C_B$), which $B$ has adopted during round $r_3$ at which $\pi_B$ was generated. Furthermore, $\pi_A$ was generated at round $r'_3 \leq r_3$.
...
%%ANDRI:
Note that we consider the parts of the proofs succeeding block $b$ are the decisive ones for the choice of the verifier. This is to adversary's advantage, since the parts preceding this block demonstrate the proof-of-work contained in common (sub)chain of $A$ and $B$, and considering chain quality for the proof of any honest player, the adversary could only include equal or less proof-of-work in her proof for this part of the chain.
...
But $\vert \alpha\{b': \} \vert = k_3  $ so we have
\begin{equation}
\vert \alpha_A\{b': \} \vert \geq \vert \alpha_A \vert - k_1
\end{equation}
\begin{equation}
\vert \alpha_A\{b': \} \vert \geq \vert \alpha_A \vert - k_2
\end{equation}
By adding (2) and (3) we have
\begin{equation*}
2\vert \alpha_A\{b': \} \vert \geq 2\vert \alpha_A \vert - (k_1 + k_2) \Rightarrow 
\end{equation*}
\begin{equation*}
\vert \alpha_A\{b': \} \vert \geq \vert \alpha_A \vert - \dfrac{1}{2}(k_1 + k_2) \Rightarrow
\end{equation*}
\begin{equation}
\vert \alpha_A\{b': \} \vert \geq \vert \alpha_A \vert - \dfrac{1}{2}(m - k_3) 
\end{equation}

By combining (1) and (4) we have
\begin{equation*}
2^{\mu_A} \vert \alpha_A\ \vert - \dfrac{2^{\mu_A}}{2}(m - k_3) < 2^{\mu'_B} \vert \alpha'_B \vert \Rightarrow
\end{equation*}
\begin{equation}
2^{\mu_A} \vert \alpha_A\ \vert < 2^{\mu'_B} \vert \alpha'_B \vert + \dfrac{2^{\mu_A}}{2}(m - k_3)
\end{equation}
